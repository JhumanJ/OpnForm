<template>
  <div>
    <h3 class="font-semibold text-2xl text-gray-900">Cài đặt không gian làm việc</h3>
    <small class="text-gray-600">Quản lý các không gian làm việc của bạn.</small>
    ...
    <modal :show="workspaceModal" @close="workspaceModal = false" max-width="lg">
      ...
      <div class="px-4">
        <form @submit.prevent="createWorkspace" @keydown="form.onKeydown($event)">
          <div>
            <text-input name="name" class="mt-4" :form="form" :required="true" label="Workspace Name" />
            <text-input name="emoji" class="mt-4" :form="form" :required="false" label="Emoji" />
          </div>

          <div class="w-full mt-6">
            <v-button :loading="form.busy" class="w-full my-3">Lưu</v-button>
          </div>
...
export default {
  ...
  methods: {
    ...
    deleteWorkspace(workspace) {
      this.alertConfirm('Bạn có chắc muốn xóa không gian làm việc này không? Tất cả các biểu mẫu được tạo trong không gian làm việc này sẽ bị xóa.', () => {
        this.$store.dispatch('open/workspaces/delete', workspace.id).then(() => {
          this.alertSuccess('Không gian làm việc bị xóa thành công.')
        })
      })
    },
    isUrl(str) {
