<template>
  ...
  <modal :show="show" @close="close" max-width="lg">
    ...
    <template #title>
      <template v-if="isMailSent">Kiểm tra email của bạn</template>
      <template v-else>Quên mật khẩu?</template>
    </template>
    <template v-if="isMailSent">
      <div class="text-center">Chúng tôi đã gửi một liên kết đặt lại mật khẩu đến <br /><span>{{ form.email }}</span></div>
      <div class="w-full p-4 text-center">
        <span class="mt-4">Không nhận được email? <a href="#" class="ml-1" @click.prevent="send">Nhấp để gửi
            lại</a></span>
      </div>
    </template>
    <template v-else>
      <div class="text-center">Đừng lo lắng, chúng tôi sẽ gửi cho bạn hướng dẫn đặt lại mật khẩu.</div>
      <form @submit.prevent="send" @keydown="form.onKeydown($event)" class="p-4">
        <text-input name="email" :form="form" label="Email" placeholder="Địa chỉ email của bạn" :required="true" />

        <div class="w-full mt-6">
          <v-button :loading="form.busy" class="w-full my-3">Đặt lại mật khẩu</v-button>
        </div>
      </form>
    </template>
    <div class="w-full text-center">
      <a href="#" @click.prevent="close" class="text-xs hover:underline text-gray-500 sm:text-sm hover:text-gray-700">
        <svg class="inline mr-1" width="15" height="14" viewBox="0 0 15 14" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path d="M13.3334 6.99996H1.66669M1.66669 6.99996L7.50002 12.8333M1.66669 6.99996L7.50002 1.16663"
