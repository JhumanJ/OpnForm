  ...
  <collapse class="p-4 w-full border-b" v-model="isCollapseOpen">
    <template #title>
      <h3 class="font-semibold text-lg relative">
        <svg class="h-5 w-5 inline mr-2 -mt-0.5 transition-colors"
          :class="{ 'text-blue-600': isCollapseOpen, 'text-gray-500': !isCollapseOpen }" viewBox="0 0 18 18" fill="none"
    ...
    <text-input name="submit_button_text" class="mt-4" :form="form" label="Văn bản của Nút Gửi" :required="true" />

    <toggle-switch-input name="editable_submissions" :form="form" class="mt-4"
      help="Cung cấp cho người dùng một liên kết duy nhất để cập nhật đợt nộp của họ">
      <template #label>
        Lượt nộp có thể chỉnh sửa
        <pro-tag class="ml-1" />
      </template>
    </toggle-switch-input>
