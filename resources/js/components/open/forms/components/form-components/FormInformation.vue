<template>
  <collapse class="p-4 w-full border-b" v-model="isCollapseOpen">
    ...
    <rich-text-area-input name="description"
                          ...
                          :required="false"
    />
    <select-input name="tags" label="Gán thẻ" :form="form" class="mt-3 mb-6"
                help="Gán thẻ sẽ thuận lợi cho việc sắp xếp biểu mẫu (không hiển thị cho người phản hồi)"
                placeholder="Chọn thẻ" :multiple="true" :allowCreation="true"
                :options="allTagsOptions"
    />
    <select-input name="visibility" label="Hiển thị" :form="form" class="mt-3 mb-6"
    ...
    <v-button color="light-gray" class="w-full" v-if="copyFormOptions.length > 0" @click="showCopyFormSettingsModal=true">
      ...
      Sao chép thiết lập từ biểu mẫu sẵn có
    </v-button>
    ...
    <modal :show="showCopyFormSettingsModal" @close="showCopyFormSettingsModal=false" max-width="md">
      ...
      <div class="p-4 min-h-[450px]">
        <p class="text-gray-600">
          Nếu bạn đã có một biểu mẫu khác mà bạn muốn sử dụng làm cơ sở cho biểu mẫu này, bạn có thể chọn ở đây. 
          Xác nhận và chúng tôi sẽ sao chép tất cả các cài đặt của biểu mẫu khác (trừ cấu trúc của biểu mẫu) sang biểu mẫu này
        </p>
        <select-input v-model="copyFormId" name="copy_form_id"
                      label="Sao chép thiết lập từ" class="mt-3 mb-6"
                      placeholder="Chọn biểu mẫu" :searchable="copyFormOptions.length > 5"
  ...
  data () {
    return {
      ...
      visibilityOptions: [
        ...
        {
          name: "Bản nháp (biểu mẫu sẽ không thể truy cập được)",
          value: "draft"
        },
        {
          name: "Đóng",
