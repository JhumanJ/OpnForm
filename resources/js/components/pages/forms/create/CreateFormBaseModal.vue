  ...
  <modal :show="show" @close="$emit('close')" :closeable="!aiForm.busy">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8" v-if="state == 'default'">
      <div class="rounded-md border p-4 flex flex-col items-center cursor-pointer hover:bg-gray-50"
        ...
        <p class="font-medium">Tạo biểu mẫu mới thủ công</p>
      </div>
      <div v-if="aiFeaturesEnabled" v-track.select_form_base="{ base: 'ai' }"
        ...
        <p class="font-medium text-blue-700">Tạo biểu mẫu bằng AI</p>
        <span class="text-xs text-gray-500">(1 phút)</span>
      </div>
      <div class="rounded-md border p-4 flex flex-col items-center cursor-pointer hover:bg-gray-50 relative">
        ...
        <p class="font-medium">Chọn hình thức tạo biểu mẫu</p>
        <router-link :to="{ name: 'templates' }" v-track.select_form_base="{ base: 'template' }"
    ...
    <div v-else-if="state == 'ai'">
      ...
      <text-area-input label="Form Description" :disabled="loading" :form="aiForm" name="form_prompt"
        help="Hãy cho chúng tôi mô tả về biểu mẫu bạn muốn xây dựng (càng chi tiết càng tốt)"
      ...
      <p class="text-gray-500 text-xs text-center mt-1">~60 giây</p>
    </div>
  </modal>
</template>
      ...
      setTimeout(() => {
        axios.get('/api/forms/ai/' + generationId).then(response => {
          if (response.data.ai_form_completion.status === 'completed') {
            ...
            this.alertError('Đã xảy ra lỗi. Vui lòng thử lại sau.')
            this.state = 'default'
            this.loading = false
          } else {
