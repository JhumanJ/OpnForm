...
4. Set up environment files:
   ...
   ```
   This script will create the necessary `.env` files for both the API and client, it will not overwrite any existing `.env`, so it's better to make sure there already no `.env` file in both the `/api` and `/client` directory.
...
9. For the frontend to communicate with the backend correctly, make sure both of `NUXT_PUBLIC_APP_URL=http://localhost:3000` and `NUXT_PUBLIC_API_BASE=http://localhost:8000` are set on `/client/.env`
