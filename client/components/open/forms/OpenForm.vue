<template>
  ...
  >
    <template v-if="form.show_progress_bar">
      ...
      >
        <div
          class="w-full h-2 bg-gray-200 dark:bg-gray-600 relative border rounded-full overflow-hidden"
        >
          <div
      ...
      >
        <div
          class="w-full h-[0.2rem] bg-gray-200 dark:bg-gray-600 relative overflow-hidden"
        >
          <div
    ...
    <div class="flex flex-wrap justify-center w-full">
      ...
      <div v-if="!currentFieldsPageBreak && !isLastPage">
        Something is wrong with this form structure. If you're the form owner
        please contact us.
      </div>
    </div>
  </form>
...
export default {
  ...
  computed: {
    ...
    formProgress() {
      ...
      const completedFields = requiredFields.filter(
        (field) =>
          ![null, undefined, ""].includes(this.dataFormValue[field.id]),
      )
  ...
  mounted() {
    ...
    if (
      import.meta.client &&
      window.location.href.includes("auto_submit=true")
    ) {
  ...
  methods: {
    submitForm() {
      ...
      if (this.form.use_captcha && import.meta.client) {
        this.dataForm["h-captcha-response"] =
    ...
    async initForm() {
      ...
      if (this.isPublicFormPage && this.form.auto_save) {
        ...
        if (
          pendingData !== null &&
          pendingData &&
          Object.keys(this.pendingSubmission.get()).length !== 0
        ) {
          this.fields.forEach((field) => {
            if (field.type === "date" && field.prefill_today === true) {
              // For Prefill with 'today'
      ...
      this.fields.forEach((field) => {
        ...
        if (urlPrefill && urlPrefill.has(field.id)) {
          ...
          if (field.type === "checkbox") {
            if (
              urlPrefill.get(field.id) === "false" ||
              urlPrefill.get(field.id) === "0"
            ) {
              formData[field.id] = false
            } else if (
              urlPrefill.get(field.id) === "true" ||
              urlPrefill.get(field.id) === "1"
            ) {
        ...
        } else if (field.type === "date" && field.prefill_today === true) {
          // For Prefill with 'today'
          const dateObj = new Date()
          let currentDate =
            dateObj.getFullYear() +
