<template>
  <form v-if="form" @submit.prevent="" class="@container w-full relative overflow-hidden flex flex-col min-h-full">
    ...
    >
      ...
      <UButton v-if="!isLast" color="form" square variant="solid" size="sm" icon="i-heroicons-chevron-down-20-solid" @click="goNext" />
    </div>
  </form>
</template>
...
const goPrev = () => { if (canGoPrev.value) props.formManager.previousPage().then(() => import.meta.client && window.scrollTo({ top: 0, behavior: 'smooth' })) }
