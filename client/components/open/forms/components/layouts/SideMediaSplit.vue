<template>
  <div class="w-full flex z-10 flex-col @4xl:flex-row">
    <!-- Mobile: media as background on top, content dictates scroll -->
    <div class="relative block @4xl:hidden w-full">
    ...
    <div v-if="isLeft" class="hidden @4xl:block w-1/2 relative overflow-hidden h-screen">
      <BlockMediaLayout :image="image" :fallback-height="null" />
    </div>

    <!-- Content -->
    <div class="w-full @4xl:w-1/2 flex items-center px-6">
      <div class="w-full max-w-2xl mx-auto mt-4 @4xl:mt-0 py-4 @2xl:px-8">
        <slot />
      </div>
    </div>
