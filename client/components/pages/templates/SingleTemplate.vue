<template>
    <div v-if="template" class="relative group">
        <div v-if="template.is_new" class="absolute top-0 right-0 p-3 z-10">
            <span class="inline-flex items-center gap-1 rounded-full bg-blue-500 px-2 py-1 text-xs font-medium text-white">
        ...
        <template-tags :slug="template.slug" class="flex mt-4 items-center flex-wrap gap-3" />
        <NuxtLink :to="'/templates/show?slug=' + template.slug">
