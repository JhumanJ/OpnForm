    ...
    <section class="bg-white py-12">
      <div class="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div v-if="templatesLoading" class="text-center mt-4">
          <CommonLoader class="h-6 w-6 text-nt-blue mx-auto" />
        </div>
    ...
    <template v-if="!onlyMy">
      <section class="py-12 bg-white border-t border-gray-200 sm:py-16">
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          ...
          <div class="grid grid-cols-1 gap-8 mt-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <NuxtLink v-for="row in types" :key="row.slug" :to="'/templates/types/show?slug=' + row.slug"
              :title="row.name"
      ...
      <section class="py-12 bg-white border-t border-gray-200 sm:py-16">
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          ...
          <div class="grid grid-cols-1 gap-8 mt-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <NuxtLink v-for="row in industries" :key="row.slug" :to="'/templates/industries/show?slug=' + row.slug"
              :title="row.name"
              class="text-gray-600 dark:text-gray-400 transition-colors duration-300 hover:text-nt-blue">
              {{ row.name }}
            </NuxtLink>
