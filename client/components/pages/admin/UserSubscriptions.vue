<template>
    <AdminCard title="Subscriptions" icon="heroicons:credit-card-16-solid">
        <UTable :loading="loading" :loading-state="{ icon: 'i-heroicons-arrow-path-20-solid', label: 'Loading...' }"
            ...
            :rows="subscriptions" class="-mx-6">
            <template #stripe_id-data="{ row }">
                <a :href="'https://dashboard.stripe.com/subscriptions/' + row.id" target="_blank"
                    class="text-xs select-all bg-purple-50 border-purple-200 text-purple-500 rounded-md px-2 py-1 border">
                    <Icon name="bx:bxl-stripe" class="h-4 w-4 inline-block" />
                    {{ row.stripe_id }}
                </a>
            </template>
            <template #status-data="{ row }">
                <span class="text-xs select-all rounded-md px-2 py-1 border"
